...
def generar_calendario(equipos_lista, semana):
    if len(equipos_lista) < 6:
        return []
    # Plantilla matemática del Round-Robin perfecto (Basada en tu Semana 1)
    plantilla = [
        [(0,1), (2,3)],         # J1
        [(4,5), (0,2)],         # J2
        [(1,3), (4,2)],         # J3
    ...
    for j_idx, jornada in enumerate(plantilla):
        for partido in jornada:
            ...
            # Invertir localía en semanas pares
            if semana % 2 == 0:
...
tab_registro, tab_tabla, tab_goleo, tab_config = st.tabs(["📝 Calendario y Partidos", "📊 Tabla de Posiciones", "⚽ Tabla de Goleo", "⚙️ Configuración"])

# --- PESTAÑA 4: CONFIGURACIÓN ---
with tab_config:
    st.subheader("Alta de Torneos y Jugadores")
    st.warning("⚠️ Para que el calendario S1/S2 coincida exactamente con tus reglas, inscribe a los 6 equipos EXACTAMENTE en este orden:\n1. Rosa\n2. Perrito\n3. Salo\n4. Pedro\n5. Enorme\n6. Jop")
    col_t, col_e = st.columns(2)
    with col_t:
        nuevo_torneo = st.text_input("Nombre del Torneo:")
    with col_e:
        nuevo_equipo = st.text_input("Emoji + Equipo + (Manager):")
    ...
    if equipos_activos:
        st.write(f"**Equipos en {torneo_actual} ({len(equipos_activos)}/6):**")
        for i, eq in enumerate(equipos_activos):
            st.write(f"{i+1}. {eq}")

# --- PESTAÑA 1: CALENDARIO Y REGISTRO ---
with tab_registro:
    if len(equipos_activos) != 6:
        st.error("⚠️ El calendario automático requiere exactamente 6 equipos inscritos en la pestaña Configuración.")
    else:
        st.subheader(f"📅 Calendario Semana {semana_actual}")
        
        # Generar partidos de la semana
        ...
        # Separar en jugados y pendientes
        ...
        for idx, p in enumerate(partidos_semana):
            ...
            # Dibujar en 2 columnas para que no se vea tan largo
        ...
        if not pendientes_para_dropdown:
            ...
            partido_seleccionado = st.selectbox("Selecciona el partido a registrar:", list(opciones_partidos.keys()))
            
            datos_partido = opciones_partidos[partido_seleccionado]
            local = datos_partido['Local']
            visita = datos_partido['Visitante']
            jornada_act = datos_partido['Jornada']
                ...
                with col_g1:
                    if goles_l > 0:
                        ...
                        for i in range(goles_l):
                            j = st.text_input(f"Gol {i+1}", key=f"g_l_{i}")
                ...
                with col_g2:
                    if goles_v > 0:
                        ...
                        for i in range(goles_v):
                            j = st.text_input(f"Gol {i+1}", key=f"g_v_{i}")
            ...
            if st.button("Guardar Resultado Oficial", type="primary"):
                ...
                st.success(f"✅ ¡Partido guardado exitosamente!")
                st.rerun()
...
with tab_tabla:
    ...
    if not partidos_torneo.empty:
        ...
        for _, p in partidos_torneo.iterrows():
            ...
            if gl > gv:
                stats[loc]['G'] += 1; stats[loc]['Pts'] += 3; stats[vis]['P'] += 1
            elif gv > gl:
                stats[vis]['G'] += 1; stats[vis]['Pts'] += 3; stats[loc]['P'] += 1
            else:
                stats[loc]['E'] += 1; stats[vis]['E'] += 1; stats[loc]['Pts'] += 1; stats[vis]['Pts'] += 1
